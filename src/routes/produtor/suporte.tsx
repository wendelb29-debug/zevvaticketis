...
function SuportePage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-manrope font-extrabold text-foreground">Suporte ao Participante</h1>
        ...
        {results.map((profile: any) => (
          <div key={profile.id} className="bg-card p-6 rounded-xl border space-y-4">
            <div className="flex items-center gap-3 border-b pb-4">
              ...
              <div>
                <h3 className="font-bold text-foreground">{profile.nome}</h3>
            ...
            <div className="space-y-2">
              ...
              {profile.orders?.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground italic">Nenhum pedido encontrado.</p>
              )}
            </div>
          </div>
        ))}
