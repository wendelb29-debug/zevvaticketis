  ...
  if (isLoading) {
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-muted font-bold uppercase tracking-widest text-[10px]">Carregando eventos...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 font-inter">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-manrope font-black text-navy tracking-tighter uppercase">📅 Meus Eventos</h1>
          <p className="text-sm text-muted font-medium">Gerencie a programação e vendas dos seus eventos.</p>
        </div>
        <Button 
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input 
            ...
            className="pl-11 h-12 rounded-xl border-line bg-white"
          />
        </div>
        <Button variant="outline" className="h-12 px-6 rounded-xl border-line text-navy font-bold gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {events?.map((event) => (
          <Card key={event.id} className="group border-line shadow-sm hover:shadow-xl transition-all duration-300 rounded-[32px] overflow-hidden bg-white">
            <div className="relative aspect-[16/9] overflow-hidden">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                ...
                {event.category && (
                  <Badge variant="outline" className="bg-white/90 backdrop-blur-sm border-none text-navy text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full">
                    {event.category}
                  </Badge>
                )}
              </div>
              <div className="absolute top-4 right-4">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-white/90 backdrop-blur-sm hover:bg-white text-navy border-none shadow-sm">
                      <MoreVertical className="w-4 h-4" />
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="w-48 rounded-xl p-2 border-line shadow-xl font-inter">
                    <DropdownMenuItem className="flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-bold text-navy cursor-pointer">
                      <Edit className="w-4 h-4 text-coral" /> Editar Evento
                    </DropdownMenuItem>
                    <DropdownMenuItem 
                      onClick={() => navigate({ to: `/eventos/${event.id}` as any })}
                      className="flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-bold text-navy cursor-pointer"
                    >
            ...
            <CardContent className="p-6 space-y-4">
              <div className="space-y-1">
                <h3 className="text-xl font-manrope font-black text-navy leading-tight line-clamp-1">{event.title}</h3>
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-line">
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-muted uppercase tracking-widest">Vendidos</p>
                  <div className="flex items-end gap-1">
                    ...
                    <span className="text-[10px] text-muted font-bold pb-1">/ {(event.ticket_types as any[])?.reduce((acc: number, t: any) => acc + (t.quantidade || 0), 0) || 0}</span>
                  </div>
                </div>
                <div className="space-y-1 text-right">
                  <p className="text-[9px] font-black text-muted uppercase tracking-widest">Receita Bruta</p>
        ...
        {events?.length === 0 && (
          <div className="col-span-full py-20 bg-white rounded-[32px] border-2 border-dashed border-line flex flex-col items-center justify-center gap-6 text-center">
            <div className="w-20 h-20 rounded-full bg-surface flex items-center justify-center text-muted">
              <FileText className="w-10 h-10" />
            </div>
            <div className="space-y-2">
              <h3 className="text-xl font-manrope font-black text-navy">Nenhum evento encontrado</h3>
              <p className="text-sm text-muted max-w-xs font-medium">Você ainda não criou nenhum evento para este projeto. Comece agora mesmo!</p>
            </div>
            <Button 
              onClick={() => navigate({ to: "/produtor/novo-evento" })}
