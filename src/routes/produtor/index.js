...
import { Plus, MapPin, Video, Smartphone, CheckCircle2, Ticket, Eye, Sparkles } from "lucide-react";
...
function ProdutorDashboard() {
    ...
    return (<div className="space-y-10 font-inter max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        ].map((item) => (<button key={item.label} onClick={() => navigate({ to: '/criar-evento' })} className="group flex flex-col items-start p-6 rounded-[24px] border border-line bg-white hover:border-gold/30 hover:shadow-xl transition-all duration-300 text-left">
      ...
      <div className="bg-white rounded-[32px] border border-line p-8 space-y-8 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {Object.keys(CATEGORY_THEMES).map((catName) => {
                ...
                return (<button key={catName} onClick={() => navigate({ to: '/eventos', search: { categoria: catName } })} className="group p-4 rounded-2xl border-2 border-line hover:border-navy transition-all text-center space-y-3 bg-surface/30 active:scale-95" style={{ borderColor: theme.accentColor + '40' }}>
                <div className="w-12 h-12 rounded-xl mx-auto flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform" style={{ backgroundColor: theme.accentColor }}>
      ...
      <div className="relative overflow-hidden rounded-[32px] p-8 sm:p-12 text-white dark-panel">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        ].map((text) => (<li key={text} className="flex items-center gap-3 font-semibold text-white/90">
          ...
          <div className="hidden lg:grid grid-cols-2 gap-4">
            {stats.map((stat) => (<div key={stat.label} className="glass-panel backdrop-blur-md p-6 rounded-2xl text-center shadow-lg bg-white/5">
      ...
      <div className="bg-white rounded-[24px] border border-line shadow-sm overflow-hidden">
        <Tabs defaultValue="events" className="w-full">
          <div className="px-8 pt-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <Button onClick={() => navigate({ to: '/criar-evento' })} className="bg-navy hover:bg-navy/90 text-white font-bold rounded-xl h-11 px-6">
          ...
          <TabsContent value="events" className="p-0 mt-6">
            <div className="border-t border-line">
              ...
              <div className="flex flex-col items-center justify-center py-20 px-8 text-center space-y-6">
                ...
                <Button variant="outline" onClick={() => navigate({ to: '/criar-evento' })} className="h-11 px-8 rounded-xl font-bold border-gold text-gold hover:bg-gold hover:text-white transition-all">
