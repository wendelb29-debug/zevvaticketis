...
function CheckinMonitorPage() {
  ...
  if (isLoading) return <div className="p-10 text-center text-muted-foreground-fg font-inter">Monitorando acessos...</div>;
  ...
  return (
    <div className="space-y-8 text-foreground animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground-fg">Acompanhamento em tempo real de entradas nos eventos Zevva.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <Card className="bg-card border-border shadow-sm">
           <CardContent className="p-6 flex items-center gap-4">
             ...
             <div>
               <p className="text-muted-foreground-fg font-bold text-xs uppercase tracking-wider">Check-ins Hoje</p>
          ...
          <tbody className="divide-y divide-border">
            {logs?.map((log: any, i: number) => (
              <tr key={i} className="hover:bg-accent/30 transition-colors">
                ...
                <td className="px-6 py-4 text-xs text-muted-foreground-fg font-inter">
                  <div className="flex items-center gap-2">
            ...
            {(!logs || logs.length === 0) && (
              <tr>
                <td colSpan={3} className="px-6 py-12 text-center text-muted-foreground-fg font-inter">Nenhum check-in registrado recentemente.</td>
              </tr>
            )}
          </tbody>
        </table>
