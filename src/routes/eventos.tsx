      ...
      <main className="pt-24 pb-20 max-w-7xl mx-auto px-6 space-y-12">
        ...
        <div className="relative h-[450px] rounded-[48px] overflow-hidden shadow-2xl group border-4 border-white">
          ...
          <div className="absolute top-8 left-8">
            <div className="px-4 py-2 bg-card/20 backdrop-blur-md rounded-full border border-white/30 text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="absolute bottom-12 left-8 right-8 text-white space-y-4">
            ...
            <div className="flex flex-wrap gap-6 items-center">
              <div className="flex items-center gap-2 bg-card/10 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12" id="tickets-section">
            ...
            <Tabs value={activeTab} className="w-full">
              <TabsContent value="tickets" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-4">
                  ...
                  <p className="text-muted-foreground font-medium">Selecione as categorias que melhor atendem sua necessidade.</p>
                </div>
                ...
                <div className="grid gap-6">
                  {ticketTypes.map((ticket: any) => (
                    <div 
                      key={ticket.id} 
                      className={cn(
                        "p-8 bg-card rounded-[40px] border-2 transition-all duration-300 flex flex-col md:flex-row md:items-center justify-between gap-8",
                        ticket?.id && (quantities[ticket.id] || 0) > 0 ? "border-coral shadow-2xl shadow-coral/5 scale-[1.02]" : "border-border hover:border-coral/20"
                      )}
                    >
                      <div className="space-y-4 flex-1">
                        <div className="space-y-1">
                          <h4 className="text-xl font-manrope font-black text-foreground">{ticket.nome}</h4>
                          <p className="text-sm text-muted-foreground font-bold leading-relaxed max-w-md">{ticket.descricao}</p>
                        </div>
                        <div className="flex flex-wrap gap-3">
                          <span className="px-3 py-1 bg-muted rounded-full text-[9px] font-black text-foreground uppercase flex items-center gap-1 border border-border">
                      ...
                      <div className="flex items-center justify-between md:flex-col md:items-end gap-6">
                        <div className="text-right">
                          <span className="block text-2xl font-manrope font-black text-foreground">
                            {ticket.valor.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}
                          </span>
                          <span className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">por pessoa</span>
                        </div>

                        <div className="flex items-center gap-4 bg-muted p-1.5 rounded-2xl border border-border shadow-inner">
                          <button 
                            onClick={() => updateQuantity(ticket.id, -1)}
                            className="w-10 h-10 flex items-center justify-center rounded-xl bg-card border border-border text-foreground font-black hover:bg-navy hover:text-white transition-all active:scale-90 disabled:opacity-30"
                            disabled={!quantities[ticket.id]}
                          >
                            <ChevronLeft className="w-5 h-5" />
                          </button>
                          <span className="w-8 text-center font-manrope font-black text-lg text-foreground">{quantities[ticket.id] || 0}</span>
                          <button 
                            onClick={() => updateQuantity(ticket.id, 1)}
                            className="w-10 h-10 flex items-center justify-center rounded-xl bg-card border border-border text-foreground font-black hover:bg-navy hover:text-white transition-all active:scale-90"
                          >
              ...
              <TabsContent value="about" className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-card p-10 rounded-[48px] border border-border shadow-sm space-y-8">
                  <h3 className="text-3xl font-manrope font-black text-foreground">Sobre esta Jornada</h3>
                  <div className="prose prose-navy max-w-none text-muted-foreground font-medium text-lg leading-relaxed whitespace-pre-line">
                    {event.description}
                  </div>
              ...
              <TabsContent value="travel" className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-8">
                  <h3 className="text-2xl font-manrope font-black text-foreground">Roteiro de Viagem</h3>
                  <div className="space-y-6 relative before:absolute before:left-10 before:top-4 before:bottom-4 before:w-1 before:bg-line before:rounded-full">
                    {itinerary.length > 0 ? itinerary.map((day: any) => (
                      <div key={day.id} className="relative pl-24 group">
                        <div className="absolute left-6 top-0 w-10 h-10 rounded-full bg-card border-2 border-coral flex items-center justify-center z-10 shadow-lg shadow-coral/20 group-hover:scale-110 transition-transform">
                          <span className="text-xs font-black text-coral">D{day.dia_numero}</span>
                        </div>
                        <div className="p-8 bg-card rounded-[32px] border border-border group-hover:border-coral/20 transition-all shadow-sm">
                          <h4 className="text-lg font-manrope font-black text-foreground mb-3">{day.titulo}</h4>
                          <p className="text-sm text-muted-foreground font-medium leading-relaxed">{day.descricao}</p>
                        </div>
                      </div>
                    )) : (
                      <div className="p-20 text-center bg-card rounded-[40px] border border-border border-dashed">
                        <Plane className="w-12 h-12 text-muted-foreground mx-auto mb-4 opacity-20" />
                        <p className="text-muted-foreground font-black uppercase text-xs tracking-widest">O roteiro detalhado será divulgado em breve.</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <TabsContent value="info" className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="p-8 bg-muted rounded-[40px] border border-border space-y-4">
                      ...
                      <h4 className="font-black text-foreground uppercase tracking-tight text-sm">Comunicação WhatsApp</h4>
                      <p className="text-xs text-muted-foreground font-bold leading-relaxed">Criaremos um grupo exclusivo para os participantes 30 dias antes do embarque para coordenação e dúvidas rápidas.</p>
                    </div>
                    <div className="p-8 bg-muted rounded-[40px] border border-border space-y-4">
                      <div className="w-12 h-12 bg-card rounded-2xl flex items-center justify-center shadow-sm">
                        <Users className="w-6 h-6 text-coral" />
                      </div>
                      <h4 className="font-black text-foreground uppercase tracking-tight text-sm">Tamanho do Grupo</h4>
                      <p className="text-xs text-muted-foreground font-bold leading-relaxed">Trabalhamos com grupos reduzidos de até 40 pessoas para garantir a melhor experiência e atenção individual.</p>
                    </div>
                 </div>
              </TabsContent>
            </Tabs>
          ...
          <aside className="space-y-8">
            <div className="sticky top-40 space-y-8">
              ...
              {/* Producer Info */}
              <div className="bg-card rounded-[40px] border border-border p-8 space-y-6">
                <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Realização</h4>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-muted rounded-2xl flex items-center justify-center border border-border font-black text-foreground text-xs">
                    {event.producer?.nome?.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <p className="text-sm font-black text-foreground uppercase">{event.producer?.nome || "Organização"}</p>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase">Produtor Verificado</p>
                  </div>
                </div>
              </div>

              {/* Help & Support */}
              <div className="bg-muted rounded-[40px] border border-border p-8 text-center space-y-6 group">
                <div className="w-16 h-16 bg-card rounded-3xl flex items-center justify-center mx-auto shadow-sm border border-border group-hover:scale-110 transition-transform">
                  <HelpCircle className="w-8 h-8 text-coral" />
                </div>
                <div className="space-y-2">
                  <h4 className="font-manrope font-black text-foreground text-lg">Precisa de ajuda?</h4>
                  <p className="text-xs text-muted-foreground font-bold leading-relaxed">Nossos especialistas estão disponíveis para te ajudar em cada passo da sua jornada.</p>
                </div>
                <Button variant="outline" className="w-full rounded-2xl h-14 font-black text-xs gap-3 border-border hover:bg-navy hover:text-white transition-all">
