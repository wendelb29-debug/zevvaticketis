...
export function Roadmap() {
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {waves.map((wave) => (<Card key={wave.id} className={`border-none shadow-soft hover-lift ${wave.status === 'Concluído' ? 'bg-emerald-50/50' : 'bg-white'}`}>
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start mb-2">
                <Badge variant={wave.status === 'Concluído' ? 'outline' : wave.status === 'Em Progresso' ? 'default' : 'secondary'} className={cn("rounded-full", wave.status === 'Concluído' && "border-emerald-500 text-emerald-600 bg-emerald-50")}>
                  {wave.status}
                </Badge>
                {wave.status === 'Concluído' ? (<CheckCircle2 className="w-5 h-5 text-emerald-500"/>) : wave.status === 'Em Progresso' ? (<Clock className="w-5 h-5 text-gold animate-pulse"/>) : (<Circle className="w-5 h-5 text-slate-300"/>)}
              </div>
              <CardTitle className="text-lg font-manrope font-bold text-navy">{wave.title}</CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="space-y-2">
                {wave.items.map((item, i) => (<li key={i} className="text-sm text-slate-600 flex items-start gap-2">
