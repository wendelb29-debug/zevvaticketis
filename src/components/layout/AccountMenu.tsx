    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="flex items-center gap-3 p-1 rounded-sm hover:bg-background transition-all outline-none group border border-transparent hover:border-border">
          ...
          <ChevronDown className="w-3 h-3 text-muted-foreground-foreground group-hover:text-primary transition-colors" />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent
        align="end"
      ...
      >
        <div className="px-3 py-4 border-b border-border mb-2">
          ...
          <p className="text-[10px] font-medium text-muted-foreground-foreground truncate">{user.email}</p>
        </div>

        <DropdownMenuItem
          onClick={() => onNavigate("/app")}
          className="flex items-center gap-3 px-3 py-2.5 text-[10px] font-bold uppercase tracking-widest cursor-pointer rounded-sm hover:bg-background text-muted-foreground-foreground hover:text-primary transition-colors focus:bg-background focus:text-primary"
        >
