  ...
  return (
    <div className="rounded-lg border border-border bg-accent/30 overflow-auto max-h-[50vh] font-mono text-[11px] leading-5">
      {lines.map((line, i) => (
        ...
        >
          <span className="select-none text-muted-foreground/60 w-6 text-right shrink-0">{i + 1}</span>
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-5 py-4 border-b border-border">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Localize rapidamente ações por tipo, usuário e intervalo de datas. Baixe e compartilhe o histórico em CSV ou PDF quando precisar.</p>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-end gap-3 px-5 py-4 border-b border-border">
          <div className="min-w-[200px] flex-1">
            <label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Pesquisar</label>
            <Input value={search} onChange={(e) => resetPage(setSearch)(e.target.value)} className="h-9 mt-1" placeholder="Ação, alvo, usuário..." />
          </div>
          <div className="w-[170px]">
            <label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Ação</label>
          ...
          <div className="w-[190px]">
            <label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Usuários</label>
          ...
          <div className="w-[180px]">
            <label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Período</label>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-accent/40 text-muted-foreground text-[11px] font-extrabold uppercase tracking-widest border-b border-border">
              <tr>
            ...
            <tbody className="divide-y divide-border/60">
              {isLoading && (
                <tr><td colSpan={5} className="px-5 py-12 text-center text-muted-foreground animate-pulse">Carregando registros...</td></tr>
              )}
                  ...
                  <tr key={log.id} className="hover:bg-primary/5 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <div className="leading-tight">
                        ...
                        <div className="text-[11px] font-mono text-muted-foreground">{log.alvo_tipo}.{log.alvo_id?.slice(0, 8)}</div>
                      </div>
                    </td>
                    <td className="px-5 py-3">
              ...
              {!isLoading && rows.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-12 text-center text-muted-foreground">Nenhum registro de auditoria encontrado.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 px-5 py-3 border-t border-border">
          ...
          <span className="text-xs text-muted-foreground mx-auto">Página {current} de {totalPages}</span>
      ...
      <Dialog open={!!diffLog} onOpenChange={(o) => !o && setDiffLog(null)}>
        <DialogContent className="max-w-4xl bg-card">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <p className="text-xs font-bold text-muted-foreground mb-1.5">Antes</p>
              <JsonView value={diffLog?.dados_antes} tone="remove" />
            </div>
            <div>
              <p className="text-xs font-bold text-muted-foreground mb-1.5">Depois</p>
