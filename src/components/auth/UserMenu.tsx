      ...
      <DropdownMenuContent align="end" className="w-64 mt-2 rounded-xl p-0 border-border shadow-2xl font-inter overflow-hidden bg-popover/95 backdrop-blur-md">
        ...
        <div className="p-1.5">
          {isChat && onStatusChange && (
            <DropdownMenuSub>
              <DropdownMenuSubTrigger className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold text-foreground cursor-pointer hover:bg-accent">
                <div className={cn(
              ...
              <DropdownMenuPortal>
                <DropdownMenuSubContent className="w-40 rounded-xl p-1.5 border-border shadow-xl font-inter bg-popover/95 backdrop-blur-md">
                  ...
                  <DropdownMenuItem onClick={() => onStatusChange('offline')} className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold text-foreground cursor-pointer">
                    <div className="w-2 h-2 rounded-full bg-muted-foreground/20" /> Offline
                  </DropdownMenuItem>
                </DropdownMenuSubContent>
              </DropdownMenuPortal>
            </DropdownMenuSub>
          ...
          <DropdownMenuItem 
            ...
            className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold text-foreground cursor-pointer hover:bg-accent"
          >
          ...
          <DropdownMenuSub>
            ...
            <DropdownMenuPortal>
              <DropdownMenuSubContent className="w-40 rounded-xl p-1.5 border-border shadow-xl font-inter bg-popover/95 backdrop-blur-md">
                <DropdownMenuItem onClick={() => setTheme('light')} className={cn("flex items-center justify-between text-xs font-bold text-foreground cursor-pointer", theme === 'light' && "bg-accent")}>
                  <div className="flex items-center gap-2"><Sun className="w-3.5 h-3.5" /> Claro</div>
                  {theme === 'light' && <Check className="w-3.5 h-3.5 text-primary" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setTheme('dark')} className={cn("flex items-center justify-between text-xs font-bold text-foreground cursor-pointer", theme === 'dark' && "bg-accent")}>
                  <div className="flex items-center gap-2"><Moon className="w-3.5 h-3.5" /> Escuro</div>
                  {theme === 'dark' && <Check className="w-3.5 h-3.5 text-primary" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setTheme('system')} className={cn("flex items-center justify-between text-xs font-bold text-foreground cursor-pointer", theme === 'system' && "bg-accent")}>
                  <div className="flex items-center gap-2"><Monitor className="w-3.5 h-3.5" /> Sistema</div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className="w-8 h-8 flex items-center justify-center rounded-lg border border-border text-foreground hover:bg-accent"
                    >
                      -
                    </button>
        ...
        {role?.label === "Produtor" && (
          ...
          {isChat && onOpenSettings ? (
            <DropdownMenuItem 
              ...
              className="flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-bold text-foreground cursor-pointer hover:bg-accent"
            >
