-- 1) Remove anonymous full-column access to ad_campaigns
DROP POLICY IF EXISTS "Public can see active campaigns" ON public.ad_campaigns;

-- Public-safe view (no financials / internal notes)
CREATE OR REPLACE VIEW public.ad_campaigns_public
WITH (security_invoker = off) AS
SELECT id, organization_id, advertiser_id, name, priority,
       start_at, end_at, timezone, targeting, frequency_cap
FROM public.ad_campaigns
WHERE status = 'ativa' AND start_at <= now() AND end_at >= now();

GRANT SELECT ON public.ad_campaigns_public TO anon, authenticated;

-- 2) Harden check-in RPC: no anonymous execution + in-function authorization
...
BEGIN
    -- Authorization: must be signed in, acting as self, and linked to the tenant/event
